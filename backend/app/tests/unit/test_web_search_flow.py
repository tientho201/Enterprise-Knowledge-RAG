...
async def test_flow():
    async with AsyncSessionLocal() as db:
        ...
        res1 = await chat_service.chat(
            user_id=user_id, message=query, conversation_id=None, search_tool=False
        )
        ...
        res2 = await chat_service.chat(
            user_id=user_id, message=query, conversation_id=None, search_tool=True
        )
        ...
        assert any(c.source_link for c in loaded_assistant_msg.citations), (
            "Loaded response citations have no links!"
        )
        assert "<!--citations:" not in loaded_assistant_msg.content, (
            "Hidden comments leaked into loaded content!"
        )
        ...
        res3 = await chat_service.chat(
            user_id=user_id, message=rag_query, conversation_id=None, search_tool=False
        )
        ...
        await db.execute(
            delete(Conversation).where(
                Conversation.id.in_(
                    [res1.conversation_id, res2.conversation_id, res3.conversation_id]
                )
            )
        )
        await db.commit()
